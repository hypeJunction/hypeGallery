...
switch ($display) {
    ...
    case 'photostream':
        ...
        echo elgg_list_entities(array('types' => 'object', 'subtypes' => array(hjAlbumImage::SUBTYPE), 'owner_guids' => $page_owner->guid, 'joins' => array("JOIN {$dbprefix}entities albcont ON e.container_guid = albcont.guid"), 'wheres' => "(albcont.container_guid IN ({$container_guids_in}))", 'list_type' => get_input('list_type', 'gallery'), 'list_type_toggle' => true, 'gallery_class' => 'gallery-photostream', 'full_view' => false, 'pagination' => true, 'limit' => get_input('limit', 20), 'offset' => get_input('offset-photostream', 0), 'offset_key' => 'offset-photostream'));
        break;
}
echo '</div>';